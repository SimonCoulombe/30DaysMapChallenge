...
rm(both_rasters_crop)
gc()
bryce_mat = raster_to_matrix(bryce)

#bryce_small <- bryce_mat
bryce_small <- resize_matrix(bryce_mat, 0.5)
...
# plot_map(final_map)
# save_png(final_map, "2021_11_3d_parc_national_de_la_gaspesie/map2d.png")

base_map %>% 
  add_overlay(polygon_layer) %>%
  add_overlay(stream_layer, alphalayer = 0.8) %>% 
  add_overlay(trails_layer) %>%
  plot_3d(bryce_zoom_mat, windowsize=c(2400,1600))

base_map %>% 
  add_overlay(polygon_layer) %>%
  add_overlay(stream_layer, alphalayer = 0.8) %>% 
  add_overlay(trails_layer) %>%
  plot_3d(zscale = 2, bryce_zoom_mat, windowsize=c(2400,1600))
